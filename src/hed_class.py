...
class hed(object):
    ...
    def createHed(self, input_img = None, train_or_test = True):
        '''if train_or_test == True:
            input_img = tf.placeholder(dtype = tf.float32, shape = (None, self.trainImHt, self.trainImWdt, 3), name= 'image')
        else:
            input_img = tf.placeholder(dtype = tf.float32, shape = (None, None, None, 3), name= 'image')'''
        
        with tf.variable_scope('conv1_1') as scope:
            x = convLayer(inputFeat=input_img, name='1', kernel_shape=[3,3,3, 64], stride_shape=[1,1,1,1])
    ...
    def trainModel(self, hedModel = None, preTrainedHedModelPath="../hedPreTrainedModel/HED_reproduced.npy",
        ...
        b1_pred, b2_pred, b3_pred, b4_pred, b5_pred, branchComb_pred  = self.createHed(input_img=x_image, train_or_test=True)
    ...
    def modelPred(self, modelFilePath = '../checkpoint/model.ckpt-165', testImgFolder = '../data/testData/', 
                  testOneFile = False, testImgPath = None, resultFolder = '../data/result/', trainDataMean = 188.9, testBatchSize = 1, resizeto=None, grayImg = None, rgbImg = None):
        ...
        '''x = tf.placeholder(tf.float32, shape=[None, 304*304*3], name="inputX")
        x_image = tf.reshape(x, [-1, 304, 304, 3])'''
        #x_image = tf.placeholder(tf.float32, shape=[None, 1008, 1008, 3], name="inputX")
        x_image = tf.placeholder(tf.float32, shape=[None, resizeto[0], resizeto[1], 3], name="inputX")
        
        _, _, _, _,_, branchComb_pred  = self.createHed(x_image, train_or_test=False)
        saver = tf.train.Saver(tf.global_variables())
        saver.restore(sess, modelFilePath)
        
        if testOneFile == False: 
            xPath = os.path.join(testImgFolder)
            xPaths = glob.glob(xPath + '/*.png')
            xPaths = sorted(xPaths)
        else:
            xPaths = [testImgPath]
        ...
        for imgPath in xPaths:
            ...
            if grayImg == 1:
                ...
                img[:,:,0] = (img_gray - trainDataMean)
                img[:,:,1] = (img_gray - trainDataMean)
                img[:,:,2] = (img_gray - trainDataMean)
            elif rgbImg == 1:
                img[:,:,0] = (img[:,:,0] - 176.27)
                img[:,:,1] = (img[:,:,1] - 132.81)
                img[:,:,2] = (img[:,:,2] - 174.67)
        ...
        testImgs = np.asarray(testImgs, dtype = np.float32)
        
        if len(testImgs)<testBatchSize: testBatchSize = 1
        
        for batchId in range(testBatchSize-1, len(testImgs), testBatchSize):
            
            oneBatchTestImgs = testImgs[batchId - testBatchSize +1: batchId+1, :,:,:]
            oneBatchTestImgNames = testImgNames[batchId - testBatchSize+1: batchId+1]
            
            cellDet = sess.run(branchComb_pred, feed_dict={x_image: oneBatchTestImgs})
            ...
            for i in range(len(cellDet)):
                ...
                if len(local_maxi)>0:
                    ...
                    filePath, imgName = oneBatchTestImgNames[i]
                    ...
                    if len(np.shape(im)) == 2:
                        ...
                        im_cv[:,:,1] = im[:,:,1].copy()
                        im_cv[:,:,2] = im[:,:,2].copy()
                    imWdt, imHt = np.float32(imWdt), np.float32(imHt)
                    temp = np.zeros((seedsOneImg.shape[0], seedsOneImg.shape[1]))        
                    temp[:,1]=seedsOneImg[:,0]*(imWdt/np.float32(resizeto[0]))
                    temp[:,0]=seedsOneImg[:,1]*(imHt/np.float32(resizeto[1]))
                    
                    seedsOneImg[:,0]=seedsOneImg[:,0]*(imWdt/np.float32(resizeto[0]))
                    seedsOneImg[:,1]=seedsOneImg[:,1]*(imHt/np.float32(resizeto[1]))
                    
                    for ss in temp:
                        cv2.circle(im_cv,tuple(np.int32(ss)), 2, (0, 255, 0), thickness=2)    
                    cv2.imwrite(resultFolder + imgName[:-3] + 'jpg', im_cv)    
                    np.save(resultFolder + imgName[:-4], seedsOneImg)
